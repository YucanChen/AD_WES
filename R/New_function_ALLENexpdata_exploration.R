...
get_exprmat_ALLEN<-function(exp_matrix,exp_colsamples,brain_region,disease_type,geneset_entrezid="NA"){
  ...
  exp_matrix %<>% filter(sum > 1) %>% select(-c("sum"))
...
draw_violin_plots<-function(zcore_signifgenes,disease_type,brain_region,fdr_threshold="NA"){
  if(fdr_threshold=="NA"){
    ...
    matrix_violinplot<-zcore_signifgenes[,1:(ncol(zcore_signifgenes)-3)] %>% cbind(.,padjust_fdr=zcore_signifgenes$padjust_fdr) %>% rownames_to_column() %>% filter(padjust_fdr < fdr_threshold) %>% column_to_rownames() %>% dplyr::select(-c("padjust_fdr"))
  }
